"""
Gemini Omni Flash Video Edit via WaveSpeed.

Endpoint: POST https://api.wavespeed.ai/api/v3/google/gemini-omni-flash/video-edit
...
class GeminiOmniService:
    """Gemini Omni Flash video-edit via WaveSpeed API."""

    def __init__(self, model: str = "gemini-omni-flash", **kwargs):
        self.model = model
        self._key = _hydrate_wavespeed_key()
        if not self._key:
            raise ValueError("WAVESPEED_API_KEY not found. Set it in your environment or .env file.")
        logger.info(f"GeminiOmniService initialized (WaveSpeed): {self.model}")

    def _endpoint(self) -> str:
        return f"{_BASE}/google/{self.model}/video-edit"
        ...
        **kwargs,
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            if r.status_code != 200:
                raise Exception(f"WaveSpeed Gemini Omni submit failed: {r.status_code} - {r.text[:400]}")
            ...
            logger.info(f"Gemini Omni submitted: {pred_id}")
    ...
    async def _poll(self, client: httpx.AsyncClient, poll_url: str,
        ...
        while time.time() - start < max_wait:
            ...
            if status in ("completed", "succeeded"):
                ...
                if outputs:
                    return outputs[0]
                raise Exception(f"Gemini Omni completed but no outputs: {d}")
            if status in ("failed", "error"):
                raise Exception(f"Gemini Omni generation failed: {d.get('error') or d}")
            await asyncio.sleep(poll_interval)
        raise TimeoutError(f"Gemini Omni timed out after {max_wait}s")
...
class GeminiOmniWrapper(ModelWrapper):
    """ModelWrapper for Gemini Omni Flash video-edit. Returns the 8 standardized fields."""

    def __init__(self, model: str = "gemini-omni-flash", output_dir: str = "./outputs", **kwargs):
        self.model = model
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True, parents=True)
        self.kwargs = kwargs
        self.service = GeminiOmniService(model=model, **kwargs)
    ...
    def generate(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.error(f"Gemini Omni generation failed: {e}")
            return {
