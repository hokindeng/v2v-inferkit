...
| `ltx-2.3-extend` | `fal-ai/ltx-2.3/extend-video` | Continuation, LTX-2.3 Pro: `duration` is a float 2–20 s of new content and follows `ground_truth.mp4` exactly; `context` = the whole source; `mode: end`. fal rejects sources under **73 frames**, so a 60-frame clip is front-padded (first frame cloned) to 3.25 s before upload — trim that prefix before scoring (`input_padded_seconds` is in the record). **The returned file is source + extension stitched** (verified 2026-08-30 by frame diffs: 3.25 s padded source + 2.5 s asked → 145 frames / 6.04 s, LTX rounds to 8k+1 frames), at **1920×1080 / 24 fps** regardless of source size — trim the source span before scoring. |
